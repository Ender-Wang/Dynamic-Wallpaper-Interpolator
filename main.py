    ...
    for i in range(len(img_paths) - 1):
        ...
        if img1 is None:
            raise FileNotFoundError(
                f"{img1_path} could not be loaded. Please check the file paths."
            )
        if img2 is None:
            raise FileNotFoundError(
                f"{img2_path} could not be loaded. Please check the file paths."
            )

        if img1.shape != img2.shape:
            raise ValueError(
                f"Images {img1_path} and {img2_path} must have the same dimensions for interpolation"
            )
        ...
        for step in range(1, num_steps + 1):
            alpha = step / (
                num_steps + 1
            )  # Adjust alpha to produce the correct number of images
            beta = 1.0 - alpha
...
def main():
    parser = argparse.ArgumentParser(
        description="Interpolate images between two images with customizable order and number of inter-images for each image-pairs.",
        allow_abbrev=False,
    )
    ...
    if len(args.num_steps) != len(args.images) - 1 and len(args.num_steps) != 1:
        parser.error(
            f"The number of num_steps should be {len(args.images) - 1}, but {len(args.num_steps)} were provided."
        )
